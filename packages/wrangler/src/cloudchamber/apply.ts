...
function containerAppToCreateApplication(
	...
	return {
		...containerApp,
		scheduling_policy:
			containerApp.scheduling_policy ?? SchedulingPolicy.REGIONAL,
		constraints: {
...
export async function applyCommand(
	...
	for (const appConfigNoDefaults of config.container_app) {
		...
		if (application !== undefined && application !== null) {
			...
			const results = Diff.diffLines(prev, now);
			...
			actions.push({
				action: "modify",
				application: createApplicationToModifyApplication(application),
				id: application.id,
				name: application.name,
			});
